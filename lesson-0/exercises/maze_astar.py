# A* Search with Heuristic function estimate by Manhattan distance and cost to reach node (path cost).
...
class Node():
    def __init__(self, state: tuple[int, int], parent, action: str, distance: int, path_cost: int):
        self.state = state
        self.parent = parent
        self.action = action
        self.distance = distance # g
        self.path_cost = path_cost # h
        self.total_cost = distance + path_cost # g + h
    ...
    def remove(self) -> Node:
        if self.is_empty():
            ...
            for node in self.frontier:
                if next_remove_node.total_cost > node.total_cost:
...
class Maze:
    ...
    def solve(self):
        ...
        start = Node(state=self.start, parent=None, action=None, distance=-1, path_cost=0)
        frontier = Frontier()
        frontier.add(start)
        ...
        while True:
            ...
            for action, state, distance in neighbors:
                if not frontier.is_contains_state(state) and state not in self.explored_states:
                    neighbor_node = Node(state=state, parent=node, action=action, distance=distance, path_cost=node.path_cost + 1)
